...
var CENTER_SHAPES = [
  ...
  { name: "Throat", type: "rectangle", cx: 410, cy: 299, w: 140, h: 88 },
  ...
  { name: "Root", type: "rectangle", cx: 410, cy: 716, w: 136, h: 84 }
];
var GATE_POSITIONS = [
  ...
  { gate: 40, x: 556, y: 392 },
  // right vertex of EGO, facing Solar Plexus
...
var CHANNEL_PATHS = [
  ...
  { gates: [17, 62], path: "M 378,224 C 374,244 372,258 368,270" },
  { gates: [43, 23], path: "M 418,224 L 438,224" },
  { gates: [11, 56], path: "M 398,224 C 394,240 389,258 385,270" },
  ...
  { gates: [37, 40], path: "M 668,498 C 628,468 596,428 556,392" },
  ...
  { gates: [9, 52], path: "M 435,574 C 430,620 424,665 419,704" },
  ...
  { gates: [18, 58], path: "M 152,570 C 50,600 40,690 365,704" },
  { gates: [28, 38], path: "M 152,552 C 70,590 75,685 383,704" },
  { gates: [32, 54], path: "M 152,534 C 90,580 100,675 401,704" },
  // ── ROOT ↔ SOLAR PLEXUS (3 channels - outer right arcs) ─────────────────────
  { gates: [19, 49], path: "M 437,704 C 530,695 630,650 668,534" },
  { gates: [39, 55], path: "M 455,704 C 548,700 640,660 668,570" },
...
function buildTheme(preset, opts) {
  ...
  if (preset === "canonical") {
    ...
    base = {
      ...
      background: "#ffffff",
      centerColors: { ...CANONICAL_CENTER_COLORS },
      definedCenter: "#cfcfcf",
      undefinedCenterStroke: "#c7c2ba",
      centerStroke: "#242424",
      centerStrokeWidth: 2,
      spreenBorder: "#00838f",
      designFill: "#d4553a",
      personalityFill: "#242424",
      bothFill: "#7a4fc2",
      inactiveFill: "#ffffff",
      inactiveText: "#8f8a84",
      pillStroke: "#242424",
      pillText: "#ffffff",
      potentialChannelStroke: "#e7e3db",
      potentialChannelWidth: 1.4,
      channelColor: (_k) => "#333333",
      channelWidth: 4,
      designText: "#d4553a",
      personalityText: "#242424"
    };
    ...
    base = {
      ...
      pillStroke: "#3a2810",
      pillText: "#ffffff",
      potentialChannelStroke: "#ddd3c2",
      potentialChannelWidth: 4,
      potentialChannelDash: "none",
      channelColor: (_k) => "#3a2810",
      channelWidth: 5,
      spineStroke: "#3d2b1a",
      spineOpacity: "0.6",
      designText: "#c87860",
...
function renderCenter(shape, defined, theme) {
  ...
  const sw = theme.centerStrokeWidth;
  ...
  switch (shape.type) {
    case "triangle-up": {
      ...
      return `<path d="M ${ax},${ay} L ${bx},${by} L ${dx},${dy} Z" fill="${fill}" stroke="${stroke}" stroke-width="${sw}" pointer-events="all"/>`;
    }
    case "diamond": {
      const hw = w / 2, hh = h / 2;
      return `<polygon points="${cx},${cy - hh} ${cx + hw},${cy} ${cx},${cy + hh} ${cx - hw},${cy}" fill="${fill}" stroke="${stroke}" stroke-width="${sw}" pointer-events="all"/>`;
    }
    case "rectangle": {
      return `<rect x="${cx - w / 2}" y="${cy - h / 2}" width="${w}" height="${h}" fill="${fill}" stroke="${stroke}" stroke-width="${sw}" pointer-events="all" rx="6"/>`;
    }
    case "triangle-right": {
      const lx = cx - w * 0.4, rx = cx + w * 0.6;
      return `<path d="M ${lx},${cy - h / 2} L ${rx},${cy} L ${lx},${cy + h / 2} Z" fill="${fill}" stroke="${stroke}" stroke-width="${sw}" pointer-events="all"/>`;
    }
    case "triangle-left": {
      const rx2 = cx + w * 0.4, lx2 = cx - w * 0.6;
      return `<path d="M ${rx2},${cy - h / 2} L ${lx2},${cy} L ${rx2},${cy + h / 2} Z" fill="${fill}" stroke="${stroke}" stroke-width="${sw}" pointer-events="all"/>`;
    }
  }
}
...
function renderGatePill(gate, x, y, coloring, theme) {
  ...
  return `<g data-gate="${gate}" data-coloring="${coloring}">
    <circle cx="${x}" cy="${y}" r="8" fill="${fill}" stroke="${theme.pillStroke}" stroke-width="0.8" stroke-opacity="${coloring === "inactive" ? "0.4" : "1"}"/>
...
var ACT_COLS = { designCX: -28, persCX: 848, vbX: -76, vbW: 972 };
function renderActivationColumns(activations, theme) {
  if (!activations) return "";
  const rowH = 42, startY = 100;
  const designParts = [], persParts = [];
  const colBot = startY + 12 * rowH + 20;
  const designCX = ACT_COLS.designCX;
  designParts.push(`<rect data-col-info="design" style="cursor:help" x="${designCX - 42}" y="${startY - 34}" width="84" height="${colBot - (startY - 34)}" fill="#ffffff" fill-opacity="0" pointer-events="all"/>`);
  designParts.push(`<text data-col-info="design" style="cursor:help" x="${designCX}" y="${startY - 18}" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" font-weight="bold" letter-spacing="1" fill="${theme.designText}">DESIGN</text>`);
  ...
  const persCX = ACT_COLS.persCX;
  persParts.push(`<rect data-col-info="personality" style="cursor:help" x="${persCX - 48}" y="${startY - 34}" width="96" height="${colBot - (startY - 34)}" fill="#ffffff" fill-opacity="0" pointer-events="all"/>`);
  persParts.push(`<text data-col-info="personality" style="cursor:help" x="${persCX}" y="${startY - 18}" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" font-weight="bold" letter-spacing="0.5" fill="${theme.personalityText}">PERSONALITY</text>`);
  for (let i = 0; i < PLANETS.length; i++) {
    const { key, symbol } = PLANETS[i];
...
function renderToSVG(chartData, options = {}) {
  ...
  for (const cp of CHANNEL_PATHS) {
    if (!isChannelActive(cp.gates, chart)) {
      ...
      parts.push(`<g data-channel="${key}" data-active="false"><path d="${cp.path}" fill="none" stroke="${theme.potentialChannelStroke}" stroke-width="${theme.potentialChannelWidth}" stroke-linecap="round" stroke-linejoin="round"/></g>`);
    }
  }
  for (const cp of CHANNEL_PATHS) {
    if (isChannelActive(cp.gates, chart)) {
      ...
      parts.push(`<g data-channel="${key}" data-active="true" style="cursor:pointer">${renderChannel(cp.path, true, color, theme.channelWidth, theme)}</g>`);
    }
  }
  for (const shape of CENTER_SHAPES) {
    ...
    parts.push(`<g ${centerId} data-defined="${defined}" style="cursor:pointer">${shapeEl}${labelEl}</g>`);
  }
  if (showGateNumbers) {
  ...
  var vbX = 0, vbW = width, outMax = maxWidth;
  if (showActivationCols && chart.activations) {
    parts.push(renderActivationColumns(chart.activations, theme));
    vbX = ACT_COLS.vbX; vbW = ACT_COLS.vbW;
    outMax = Math.round(maxWidth * vbW / width);
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${vbX} 0 ${vbW} ${height}" width="100%" style="max-width:${outMax}px;height:auto;display:block;">
${parts.join("\n")}
</svg>`;
}
